...
#insert into moviesgenrescores select rt.movieId as movie_id, sum(rt.rating) as score, 19 as genre_id from ratings as rt inner join movie_genre as mg on rt.movieId = mg.movie_id where mg.genre_id = 19 group by rt.movieId order by score desc limit 1000;
...
select movie_id, max(score) as score from moviesgenrescores group by movie_id order by score desc;
...
select movie_id, max(score) as score from moviescoresaugmented group by movie_id order by score desc;
